...
var tests = new (string Name, Action Run)[]
{
    ...
    ("timing buffer uses actual runtime safety mapping", IntegratedBufferSafetySelfTests.TimingReservationUsesActualRuntimeSafetyMapping),
    ("chase buffer keeps exact intent while closing range", IntegratedBufferSafetySelfTests.ChaseReservationKeepsExactIntentWhileClosingRange),
    ("buffer Guard CC and context failures remain blocked", IntegratedBufferSafetySelfTests.GuardCrowdControlAndContextFailuresRemainBlocked),
    ...
    ("SAM late facing is opt-in bounded and resettable", SamuraiSettingsSelfTests.LateFacingIsOptInBoundedAndResettable),
    ("SAM native request and observed cast have no protection gap", SamuraiCastProtectionCoordinatorSelfTests.NativeRequestAndObservedCastHaveNoProtectionGap),
    ("SAM native queue continuation cannot block itself", SamuraiCastProtectionCoordinatorSelfTests.ExactNativeQueueContinuationDoesNotBlockItself),
    ("SAM queue ownership requires exact bounded native proof", SamuraiCastProtectionCoordinatorSelfTests.QueueOwnershipRequiresExactBoundedNativeProof),
    ("SAM rejected faulted and revoked requests cannot resurrect", SamuraiCastProtectionCoordinatorSelfTests.RejectedFaultedOrRevokedRequestsCannotResurrect),
    ("SAM emergency actions and native cast release remain authoritative", SamuraiCastProtectionCoordinatorSelfTests.EmergencyActionsAndExactCastReleaseRemainAuthoritative),
    ("SAM startup is bounded and unreviewed actions stay native", SamuraiCastProtectionCoordinatorSelfTests.StartupIsBoundedAndUnreviewedActionsRemainNative),
    ("actual helper Guard scope leaves fresh manual input alone", PluginOwnedGuardBoundarySelfTests.ActualHelperScopeBlocksGuardButLeavesManualInputAlone),
    ("all owned sources recheck Guard and honor exact escapes", PluginOwnedGuardBoundarySelfTests.EveryOwnedSourceChecksCurrentGuardAndHonorsExactEscape),
    ("uncertain Guard blocks only exact owned requests", PluginOwnedGuardBoundarySelfTests.UncertainGuardBlocksOnlyOwnedRequestsAndNestedScopesRestore),
    ("late manual Guard wins Recup and Guardian preflight", RecoveryGuardSelfTests.ManualGuardWinsLateRecuperateAndGuardianPreflight),
    ("accepted propagation and whole Guard block recovery", RecoveryGuardSelfTests.AcceptedPropagationAndWholeGuardBlockRecovery),
    ("Guard preflight read failure spends no attempt", RecoveryGuardSelfTests.GuardPreflightReadFailureDoesNotBecomeAnAttempt),
    ("Guardian resource settings default, clamp and reset", RecoveryGuardSelfTests.GuardianFallbackSettingsDefaultClampAndReset),
    ("Guardian Quick Chat keeps localized native commands", GuardianCommunicationSelfTests.GuardianQuickChatKeepsReviewedLocalizedCommands),
    ("Guardian Quick Chat retries only before native invocation", GuardianCommunicationSelfTests.GuardianQuickChatRetriesOnlyBeforeNativeInvocation),
    ("PLD late safety changes veto the native stun attempt", PaladinInterveneBoundarySelfTests.SafetyChangesBeforeDispatchVetoNativeAttempt),
    ("other counter helpers do not read PLD-only safety", PaladinInterveneBoundarySelfTests.OtherCountersDoNotReadPaladinSafety),
    ("queued helper ownership requires changed exact invoked tuple", QueuedHelperGuardOwnershipSelfTests.CaptureRequiresChangedExactInvokedHelperQueue),
    ("queued helpers remain guarded across repeated vetoes", QueuedHelperGuardOwnershipSelfTests.QueuedHelperRemainsProtectedAcrossRepeatedGuardVetoes),
    ("exact queued helpers remain attributed until Guard ends", QueuedHelperGuardOwnershipSelfTests.ExactQueuedHelperRemainsAttributedUntilGuardEnds),
    ("live queued helpers outlive attribution timers", QueuedHelperGuardOwnershipSelfTests.LiveQueuedHelperOutlivesTimerWithoutInventingGuard),
    ("exact queue ownership does not claim native success", QueuedHelperGuardOwnershipSelfTests.ChangedExactQueueOwnershipDoesNotClaimNativeSuccess),
    ("queued Guard activation ignores only its own propagation", PluginOwnedGuardBoundarySelfTests.ExactQueuedGuardContinuationRequiresLiveGuardToVeto),
    ("ambiguous Guard repeats preserve prior accepted proof", RecoveryGuardSelfTests.AmbiguousRepeatPreservesOriginalAcceptedGuardProof),
    ("queued helper attribution is bounded and manual intent wins", QueuedHelperGuardOwnershipSelfTests.QueueClearedBeforeReplayIsBoundedAndFreshManualOverrides),
    ("queued helper ownership rejects context and tuple drift", QueuedHelperGuardOwnershipSelfTests.QueueReplacementContextAndInvocationDriftInvalidate),
    ("old queue completion cannot clear a newer helper", QueuedHelperGuardOwnershipSelfTests.OldCompletionCannotClearNewHelperQueue),
};
...
static void NativeMatchupCountersUseEnemyEncounters()
{
    WithStore((store, directory) =>
    {
        ...
        Equal(0L, enemyOne.WinsTogether, "opposing win never counts as a win together");
        ...
        Equal(0L, allyOne.LossesTogether, "later opposing loss never becomes loss together");
...
static void PlayerSnapshotsRespectIdentityEpoch()
{
    ...
    WithPreparedStore(CreateOldSchemaFiveRoleHistoryDocument, (store, _) =>
    {
        True(store.StorageAvailable, "old schema-5 without optional teammate fields remains readable");
        var old = store.GetPlayerStatisticsSnapshot(1003).Players.Single();
        Equal(4L, old.WinsAgainst, "old opponent wins preserved");
        Equal(3L, old.LossesAgainst, "old opponent losses preserved");
        ...
        True(store.TryGetObservedPlayerRecord(1003, "Remote Alpha", 21, false, true, out var aggregate),
            "old prediction aggregate still available");
...
static void SearchableHistoryReloadsWithPersistedSalt()
{
    WithStore((store, directory) =>
    {
        ...
        Equal(9, store.GetPlayerStatisticsSnapshot(1003).Players.Length, "initial opponent and teammate snapshot");
        ...
        var allyOne = snapshot.Players.Single(static player => player.PlayerName == "Ally One");
        Equal(1L, allyOne.WinsTogether, "ally-only wins survive persisted salt validation");
        Equal(0L, allyOne.WinsAgainst, "ally-only row never gains opponent wins on reload");
        var enemyOne = snapshot.Players.Single(
            static player => player.PlayerName == "Enemy One");
        ...
        var updatedAlly = reloaded.GetPlayerStatisticsSnapshot(1003).Players.Single(
            static player => player.PlayerName == "Ally One");
        Equal(1L, updatedAlly.WinsTogether, "post-restart teammate win preserved");
        Equal(1L, updatedAlly.LossesTogether, "post-restart teammate loss recorded separately");
    });
}
...
static void ImportIsOneShotAndSearchable()
{
    WithStore((store, directory) =>
    {
        ...
        Equal(1L, snapshot.Players[0].WinsTogether, "imported player wins minus local losses against gives together wins");
        Equal(0L, snapshot.Players[0].LossesTogether, "imported player losses minus local wins against gives together losses");
    ...
    WithStore((store, directory) =>
    {
        True(store.TryGetPvpStatsImportPlan(1003, out var plan), "ally-only import plan");
        var import = ImportResult() with
        {
            Players = [new PvpStatsObservedPlayerAggregate("Ally Only", 21, 2, 1, 0, 0, 3, 40_000)],
        };
        True(store.TryMergePvpStatsHistory(1003, plan.StoreGeneration,
            plan.ImportBeforeUnixSecondsExclusive, import, out _), "ally-only fresh import is retained");
        var reloaded = new CrystallineConflictMapStatisticsStore(directory, null!);
        True(reloaded.StorageAvailable, "ally-only imported name survives exact HMAC validation on reload");
        var ally = reloaded.GetPlayerStatisticsSnapshot(1003).Players.Single();
        Equal("Ally Only", ally.PlayerName, "ally-only imported identity is searchable locally");
        ...
        Equal(0L, ally.WinsAgainst, "ally-only import never invents opponent wins");
        Equal(0L, ally.LossesAgainst, "ally-only import never invents opponent losses");
    });
}
...
static void PvpStatsBackfillIsIdempotent()
{
    WithPreparedStore(
        ...
        (store, _) =>
        {
            ...
            Equal(1L, firstSnapshot.Players[0].WinsTogether, "safe import backfill adds exact teammate split");
            False(
...
static string CreateOldSchemaFiveRoleHistoryDocument(string directory)
{
    ...
    var json = $$"""
    {
      ...
      "Characters": {
        "{{characterKey}}": {
          ...
          "ObservedPlayers": {
            "{{playerKey}}": {
              "PlayerName": "Remote Alpha", "WorldId": 21,
              "Wins": 20, "Losses": 10, "WinsAgainst": 4, "LossesAgainst": 3,
              "LastSeenUnixSeconds": 49000
            }
          },
          "RecentResults": [],
          "PlayerHistoryStartedAtUnixSeconds": 48000
        }
      }
    }
    """;
