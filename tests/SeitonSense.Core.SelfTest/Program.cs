...
var tests = new (string Name, Action Run)[]
{
    ...
    ("aggressor arrow overall visual scale is visible bounded and finite", AggressorArrowSelfTests.OverallVisualScaleIsVisibleBoundedAndFinite),
    ...
    ("official CC ranks parse exact published identity and snapshot", OfficialCrystallineConflictRankSelfTests.VerifiedFullPageParsesIdentityTierAndSnapshot),
    ("official CC rank fragments never infer missing metadata", OfficialCrystallineConflictRankSelfTests.PartialPageNeverInventsSnapshotOrUnlistedRanks),
    ("official CC malformed rows cannot borrow nearby fields", OfficialCrystallineConflictRankSelfTests.MalformedRowsCannotBorrowAnAdjacentIdentityOrTier),
    ("official CC conflicting rows remain unknown", OfficialCrystallineConflictRankSelfTests.ContradictoryRowsRemainUnknownRatherThanPickingAWinner),
    ("official CC rank parsing bounds and metadata fail closed", OfficialCrystallineConflictRankSelfTests.InputBoundsAndAmbiguousMetadataFailClosed),
    ("official CC rank regions use exact whitelisted selectors", OfficialCrystallineConflictRankSelfTests.RegionWhitelistMapsOnlyKnownOfficialDataCenters),
    ("official CC combined snapshot requires exact one-to-one identities", OfficialCrystallineConflictRankSelfTests.CombinedSnapshotRequiresOneToOneIdentityAndCharacterIds),
    ("official CC rank refresh is daily and rejects future timestamps", OfficialCrystallineConflictRankSelfTests.RefreshCadenceUsesElapsedDayAndRejectsFutureTimestamps),
    ...
    ("manual Guardian danger uses existing thresholds and fresh pressure", PaladinGuardianMacroSelfTests.DangerThresholdsUseExistingRiskAndFreshPressure),
    ("manual Guardian danger keeps existing ranking", PaladinGuardianMacroSelfTests.DangerSelectionPreservesExistingRanking),
    ("manual Guardian danger uses full native range before nearby fallback", PaladinGuardianMacroSelfTests.DangerUsesFullNativeRangeBeforeNearbyFallback),
    ("manual Guardian fallback chooses closest within six yalms", PaladinGuardianMacroSelfTests.FallbackSelectsClosestWithinSixYalms),
    ("manual Guardian invalid targets never enter either route", PaladinGuardianMacroSelfTests.InvalidTargetsNeverBecomeDangerOrFallback),
    ("manual Guardian fallback ties preserve exact identity", PaladinGuardianMacroSelfTests.FallbackTiesRemainExactAndDeterministic),
    ...
    ("CC medicine-kit beacon clips visible pillars and preserves height", CrystallineConflictMedicineKitSelfTests.BeaconClippingPreservesVisiblePillarsAndMinimumHeight),
    ("CC medicine-kit beacon rejects unknown and offscreen geometry", CrystallineConflictMedicineKitSelfTests.BeaconProjectionRejectsUnknownOrNonIntersectingAnchors),
    ...
    ("Smart Action evaluator preserves ordinary protection policy", SmartActionProtectionEvaluatorSelfTests.OrdinaryProtectionDecisionsMatchLegacyPolicy),
    ("Smart Action evaluator preserves utility and Ogi precedence", SmartActionProtectionEvaluatorSelfTests.UtilityAndOgiBranchOrderMatchesLegacyPolicy),
    ("Smart Action evaluator freezes target and explicit rejection", SmartActionProtectionEvaluatorSelfTests.FrozenDecisionRejectsProtectionWithoutRetargeting),
    ...
    ("opponent LB preview and live display modes never mix", OpponentLimitBreakGaugeSelfTests.PreviewAndLiveDisplayModesNeverMix),
    ...
    ("ally target arrows are hostile-only one-shot changes", AllyTargetArrowSelfTests.TargetChangesAreOneShotAndNeverPointAtFriends),
    ("ally target arrows reject stale ambiguous and reused identities", AllyTargetArrowSelfTests.ContextDeathDuplicatesAndIdentityReuseFailClosed),
    ("ally target arrows allow shared enemy focus", AllyTargetArrowSelfTests.MultipleAlliesMayFocusTheSameExactEnemy),
    ("arrow duration migration preserves customized values", AllyTargetArrowSelfTests.LongerDurationMigrationPreservesCustomSettings),
    ("aggro arrows baseline and direct transitions are one-shot", AggressorArrowSelfTests.BaselinesAndDirectTransitionsAreOneShot),
    ("aggro arrows require fresh harmful timestamps and dedupe hits", AggressorArrowSelfTests.HarmfulEvidenceRequiresFreshTimeAndDeduplicatesHits),
    ("aggro arrows reject stale context and ambiguous identities", AggressorArrowSelfTests.ContextIdentityAndPublicationGapsFailClosed),
    ("aggro arrows bound markers eligibility and pulse retention", AggressorArrowSelfTests.EligibilityMarkersNewActorsAndRetentionAreBounded),
    ("aggro arrow alpha and projected geometry fail closed", AggressorArrowSelfTests.AlphaAndProjectionBoundsRejectInvalidGeometry),
    ...
    ("SAM cast starters prefer range margin without changing instant priority", SamuraiSeitonTargetSelectionSelfTests.CastStartersPreferRangeMarginWithoutChangingInstantPriority),
